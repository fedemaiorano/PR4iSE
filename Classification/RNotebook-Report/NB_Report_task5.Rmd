---
title: "Classificazione con Naive Bayes - Task di classificazione multi-class 5"
output:
  html_document:
    df_print: paged
---
...
Si riporta di seguito la classificazione effettuata tramite Naive Bayes per il task di classificazione multi-class 5. Le classi per questo task sono le seguenti:
...
Osservando i valori di accuracy registrati dalla classificazione, si nota come le prestazioni migliori si registrino utilizzando gli ultimi due set di feature, quelli composti dalle feature di Mairesse a cui sono stati aggiunti gli n-grammi. Con entrambi i set si raggiungono identici risultati: l'accuracy media sui cinque tratti equivale a 0.668, per l'Openness a 0.64 (uguale valore si ottiene con il set di feature Mairesse con NLoN), per la Conscientiousness a 0.69, per l'Extraversion a 0.75, per l'Agreeableness a 0.62 e per il Neuroticism a 0.64. 

Con gli ultimi due set la classificazione riporta anche gli stessi valori di MAE e RMSE. Il MAE medio più basso calcolato è pari a 0.528, mentre il RMSE è 0.958. 
