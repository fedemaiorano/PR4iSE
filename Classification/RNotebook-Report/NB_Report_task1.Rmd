---
title: "Classificazione con Naive Bayes - Task di classificazione binario 1"
output:
  html_document:
    df_print: paged
---
...
Si riporta di seguito la classificazione effettuata tramite Naive Bayes per il task di classificazione binario 1, secondo il quale sono stati considerati come valori high quelli maggiori di 1 per la deviazione standard sopra la media (>1\*SD+mean), e valori low quelli minori di 1 per la deviazione standard sotto la media (<1*SD-mean).
...
Osservando i valori di accuracy ottenuti dalla classificazione, si nota come le prestazioni migliori si siano raggiunte utilizzando come set di feature quello composto dalle feature Mairesse, dagli n-grammi e pre-processing NLoN. La media delle accuratezze è pari a 0.644. Con una media di 0.61, il secondo miglior risultato è raggiunto utilizzando il set di feature composto dalle feature Mairesse e pre-processing NLoN. Gli altri due set di feature riportano accuracy peggiori (0.526 per feature Mairesse senza pre-processing NLoN, 0.564 per feature Mairesse con n-grammi senza pre-processing NLoN). 

Analizzando le accuracy per singolo tratta di personalità, si osserva come per l'Openness, per i 4 differenti set di feature, l'accuracy ottenuta equivale sempre a 0.6. Per il tratto della Conscientiousness, l'accuratezza migliore (0.75) è raggiunta con i set composti dalle feature Mairesse con n-grammi, con e senza pre-processing NLoN. Minore l'accuracy conseguita con le sole feature Mairesse senza NLoN (0.50) e con le feature Mairesse con NLoN (0.25). Per l'Extraversion si raggiunge l'accuracy di 1 usando il set di feature Mairesse con NLoN, mentre un valore di 0.67 si registra con i set composti dalle feature Mairesse con n-grammi, con e senza pre-processing NLoN. Per il set di feature Mairesse senza NLoN l'accuratezza equivale a 0.33. Misurando l'Agreeableness il risultato migliore (0.8) si ottiene con le feature Mairesse senza NLoN. Più bassi sono i risultati con feature Mairesse con NLoN e feature Mairesse con n-grammi con NLoN (entrambi 0.6), e con feature Mairesse con n-grammi senza NLoN (0.2). Infine, per il Neuroticism il valore dell'accuratezza è uguale a 0.6 per 3 set di feature, escluso il set di feature Mairesse senza NLoN per il quale è pari a 0.4. 
...
Osservando i valori di MAE, si nota come, in media, il tasso di errore più basso (0.356) è raggiunto con il set di feature composto dalle feature Mairesse, dagli n-grammi e pre-processing NLoN. Il secondo MAE più basso (0.390), in media sui 5 tratti, è raggiunto con il set di feature composto dalle feature Mairesse e pre-processing NLoN. Gli altri due set di feature riportano un tasso di errore maggiore (0.474 per feature Mairesse senza pre-processing NLoN, 0.436 per feature Mairesse con n-grammi senza pre-processing NLoN).
...
Per il RMSE il valore minore (0.552) è ottenuto con il set di feature composto dalle feature Mairesse e pre-processing NLoN. Al secondo posto il RMSE medio (0.594) raggiunto con il set di feature composto dalle feature Mairesse, dagli n-grammi e pre-processing NLoN. Valori più alti si registrano con le feature Mairesse senza pre-processing NLoN (0.676) e con le feature Mairesse con n-grammi senza pre-processing NLoN (0.646).
