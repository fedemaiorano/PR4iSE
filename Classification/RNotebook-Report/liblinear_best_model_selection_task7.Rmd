---
title: "Best LibLineaR model selection - Task di classificazione multi-class 7"
output:
  html_document:
    df_print: paged
...
In questo report sono mostrati i risultati della classificazione con best model selection tra gli 8 modelli appena elencati per il task di classificazione numero 7.
...
* Best model selection - Task di classificazione multi-class 7
...
kable(metrics_df[[1]], caption = 'Accuracy con best model selection liblinear task 7')%>%
        kable_styling("striped") %>%
        add_header_above(c(" " = 1, "Accuracy" = 6))

kable(metrics_df[[2]], caption = 'MAE con best model selection liblinear task 7')%>%
        kable_styling("striped") %>%
        add_header_above(c(" " = 1, "MAE" = 6))

kable(metrics_df[[3]], caption = 'RMSE con best model selection liblinear task 7')%>%
        kable_styling("striped") %>%
        add_header_above(c(" " = 1, "RMSE" = 6))
```

Osservando i valori di accuracy medi ottenuti, si nota come per i 4 set di feature si raggiungano valori nell'intervallo tra 0.408 e quasi 0.476. Le prestazioni migliori (accuracy pari a 0.476) si registrano utilizzando come set di feature quello composto dalle feature Mairesse con n-grammi e pre-processing NLoN. Con i primi due set, quelli formati dalle sole feature di Mairesse, il valore medio di accuracy si attesta sullo 0.42 circa. Per l'insieme fatto dalle feature Mairesse con n-grammi senza NLoN l'accuracy riportata risulta la più bassa, ed equivale a 0.408.

Andando a vedere le accuracy per singolo tratto di personalità, il valore di accuracy più alto per l'Openness (0.67) è ottenuto con l'ultimo set di feature. Per la Conscientiousness ed l'Extraversion i primi due set si comportano meglio: l'accuracy per la Conscientiousness è pari a 0.45, mentre per l'Extraversion equivale a 0.55 (stesso risultato raggiunto dal set di feature Mairesse con n-grammi e NLoN). Per l'Agreeableness con tutti i set, tranne il terzo, l'accuracy ottenuta è uguale a 0.42. Infine, per il tratto del Neuroticism, si registra un valore di 0.46 usando come insieme di feature quello composto dalle feature di Mairesse con pre-processing NLoN. 

I valori medi di MAE e RMSE sono abbastanza elevati, e risultano tutti maggiori di 1.