---
title: "Best LibLineaR model selection - Task di classificazione multi-class 4"
output:
  html_document:
    df_print: paged
...
In questo report sono mostrati i risultati della classificazione con best model selection tra gli 8 modelli appena elencati per il task di classificazione numero 4.

Per quanto riguarda la classificazione, si è diviso il dataset in training set (70%) e test set (30%). In seguito si è eseguita un fase di tuning volta a stabilire quale sia il modello liblineaR, e il relativo parametro di costo C, per il quale si registra l'accuracy maggiore nel predire i valori del test set. Si è poi ri-addestrato poi il modello, generando la matrice di confusione con le predizioni sul test set, calcolando poi le metriche di accuracy, MAE e RMSE.
...
* Best model selection - Task di classificazione multi-class 4
...
kable(metrics_df[[1]], caption = 'Accuracy con best model selection liblinear task 4')%>%
        kable_styling("striped") %>%
        add_header_above(c(" " = 1, "Accuracy" = 6))

kable(metrics_df[[2]], caption = 'MAE con best model selection liblinear task 4')%>%
        kable_styling("striped") %>%
        add_header_above(c(" " = 1, "MAE" = 6))

kable(metrics_df[[3]], caption = 'RMSE con best model selection liblinear task 4')%>%
        kable_styling("striped") %>%
        add_header_above(c(" " = 1, "RMSE" = 6))
```

Osservando i valori di accuracy medi ottenuti per set di feature, si nota come le prestazioni migliori (accuracy pari a 0.624) si siano raggiunte utilizzando come set di feature quello composto dalle feature Mairesse con n-grammi con pre-processing NLoN. Un valore medio leggermente minore è ottenuto usando il set di feature composto dalle sole feature Mairesse e pre-processing NLoN (accuracy pari a 0.6).  Peggiori sono le performance con il set di feature Mairesse senza NLoN (0.532) e con il set di feature Mairesse con n-grammi senza pre-processing NLoN (0.51).

Analizzando le accuracy per singolo tratto di personalità, il valore di accuracy più alto per l'Openness (0.67) e il Neuroticism (0.78) si è ottenuto con il set composto dalle feature Mairesse, gli n-grammi e pre-processing con NLoN. Per il tratto della Conscientiousness si è registrato lo stesso valore con tutti i set di feature (0.67). Per l'Extraversion (0.78) e l'Agreeableness (0.67) il set che si comporta meglio è quello formato dalle sole feature Mairesse con NLoN. Possiamo quindi affermare che i set in cui si è effettuato il pre-processing con NLoN classifichino meglio degli altri due. 

Osservando i valori di MAE, si nota come, in media, il tasso di errore più basso (0.556) è raggiunto con il set composto dalle feature Mairesse con n-grammi ed NLoN. Leggermente maggiore il MAE medio del secondo set di feature (0.6). Con il primo ed il terzo set, quelli senza pre-processing con NLoN, invece, il MAE medio sui cinque tratti equivale rispettivamente. a 0.712 e 0.756.

Riguardo il RMSE il valore minore (0.934) è ottenuto con l'ultimo set di feature, mentre con il set composto dalle feature Mairesse con NLoN il RMSE è uguale a 0.966, per quello formato dalle feature Mairesse senza NLoN equivale a 1.086, e per il tezo a 1.116.