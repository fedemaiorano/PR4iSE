---
title: "Classificazione con Naive Bayes - Task di classificazione multi-class 6"
output:
  html_document:
    df_print: paged
---
...
Si riporta di seguito la classificazione effettuata tramite Naive Bayes per il task di classificazione multi-class 6. Le classi per questo task sono le seguenti:
...
Osservando i valori di accuracy registrati dalla classificazione, si nota come le prestazioni migliori (accuracy media pari a 0.42) si registrano utilizzando come set di feature quello composto dalle feature Mairesse con n-grammi senza pre-processing NLoN. Un valore medio minore è ottenuto usando il set di feature composto dalle feature Mairesse con n-grammi con pre-processing NLoN (accuracy pari a 0.37).  Peggiori sono le performance di accuratezza con i due set senza gli n-grammi: con le sole feature Mairesse l'accuracy media sui cinque tratti è pari a 0.262, mentre con NLoN equivale a 0.28.

Analizzando le accuracy per singolo tratto di personalità, si osserva come con il set formato dalle feature Mairesse con n-grammi senza pre-processing con NLoN si ottengano i risultati migliori per i tratti di Openness (0.42), Conscientiousness (0.46), Extraversion (0.46, stesso valore anche con l'ultimo set di feature), Agreeableness (0.38, risultato condiviso con il primo ed il quarto insieme di feature) e Neuroticism (0.38, stesso valore anche con l'ultimo set di feature). 

Osservando i valori di MAE, si nota come, in media, il tasso di errore più basso (0.83) è raggiunto con il penultimo set di feature. Lo stesso si nota con la metrica di RMSE, dove con lo stesso set di feature si ottiene il valore minore (1.138). Di poco maggiori sono i valori riportati con il penultimo set, 0.928 di MAE medio e 1.208 di RMSE medio.
