---
title: "Classificazione con Naive Bayes - Task di classificazione binario 2"
output:
  html_document:
    df_print: paged
---
...
Si riporta di seguito la classificazione effettuata tramite Naive Bayes per il task di classificazione binario 2, secondo il quale sono stati considerati come valori high quelli maggiori di 0.5 per la deviazione standard sopra la media (>0.5\*SD+mean), e valori low quelli minori di 0.5 per la deviazione standard sotto la media (<0.5*SD-mean).
...
Osservando i valori di accuracy ottenuti dalla classificazione, si nota come con i 4 diversi set di feature si raggiunge un valore medio sui 5 tratti compreso tra 0.54 e quasi 0.6. Il valore più alto (0.592) è riportato con ìl set di feature composto dalle feature Mairesse e pre-processing NLoN, mentre quello minore con il set composto dalle sole feature Mairesse senza NLoN.

Analizzando le accuracy per singolo tratta di personalità, si osserva come con il set di feature Mairesse con NLoN si raggiungano le accuracy migliori per quanto riguarda l'Openness (0.75), l'Extraversion (0.71) e l'Agreeableness (0.67). Per Conscientiousness (0.50) e Neuroticism (0.33), invece, gli altri set di feature conseguono valori leggermente più alti. Per il primo tratto con tutti gli atri 3 set di feature si ottiene un valore di 0.75, mentre per il Neuroticism con tutti e 3 un risultato pari a 0.56. 

Da notare come non vi siano differenze nei risultati tra il set di feature composto dalle feature Mairesse con n-grammi senza NLoN e il set di feature composto dalle feature Mairesse con n-grammi con NLoN.
...
Osservando i valori di MAE, si nota come, in media, il tasso di errore più basso (0.408) è raggiunto con il set di feature composto dalle feature Mairesse con pre-processing NLoN. Il secondo MAE più basso (0.416), in media sui 5 tratti, è raggiunto con i set di feature composti dalle feature Mairesse con n-grammi, con e senza pre-processing NLoN. Il tasso più alto (0.46) è registrato usando il set formato dalle feature Mairesse senza pre-processing NLoN.
...
Per quanto concerne il RMSE, il valore minore (0.628), in media sui tratti, è ottenuto con il set di feature composto dalle feature Mairesse e pre-processing NLoN. Al secondo posto troviamo il RMSE (0.642) registrato con i set di feature composti dalle feature Mairesse con n-grammi, con e senza NLoN. Il valore più alto (0.672) si nota con le sole feature Mairesse senza pre-processing NLoN.
